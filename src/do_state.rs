...
use crate::config::{
    CredentialConfig, CredentialUpsertInput, CredentialUsageSnapshot, DurableStateDoc,
};
use crate::oauth::{
    OAuthCallbackInput, OAuthStartInput, RefreshError, exchange_code_for_tokens,
    fetch_oauth_profile, fetch_usage, maybe_refresh_access_token, oauth_start,
...
impl SgproxyState {
    ...
    async fn list_credentials(&self) -> Result<Response> {
        ...
        Ok(Response::from_json(&doc.credentials)?)
    }
    ...
    async fn create_credential(&self, payload: CredentialUpsertInput) -> Result<Response> {
        ...
        let credential = upsert_credential(&mut doc, resolved, None);
        doc.normalize(now_unix_ms());
        save_doc(&self.state.storage(), &doc).await?;
    ...
    async fn update_credential(
        ...
        let mut doc = load_doc(&self.state.storage()).await?;
        let existing = doc.credentials.iter().find(|item| item.id == id).cloned();
        if existing.is_none() {
        ...
        let resolved = self
            ...
            .await?;
        let credential = upsert_credential(&mut doc, resolved, Some(id));
        doc.normalize(now_unix_ms());
        save_doc(&self.state.storage(), &doc).await?;
        Ok(Response::from_json(&credential)?)
    }
    ...
    async fn oauth_start(&self, payload: OAuthStartInput) -> Result<Response> {
        let started = oauth_start(payload);
        let mut doc = load_doc(&self.state.storage()).await?;
    ...
    async fn oauth_callback(&self, payload: OAuthCallbackInput) -> Result<Response> {
        ...
        let mut doc = load_doc(&self.state.storage()).await?;
        let oauth_state = take_oauth_state(&mut doc, requested_state.as_deref())?;
        let token = exchange_code_for_tokens(&oauth_state, &code).await?;
        ...
        let credential = upsert_credential(&mut doc, input, None);
        doc.normalize(now_unix_ms());
        save_doc(&self.state.storage(), &doc).await?;
        ...
        match result {
            Ok(outcome) => {
                match outcome.status_code {
                    ...
                    429 => match fetch_usage(&selected.access_token).await {
                        Ok(usage) => {
    ...
    async fn resolve_proxy_credential(
        ...
        loop {
            doc.normalize(now_unix_ms());
            let selected = first_usable(&doc.credentials, now_unix_ms())
                ...
                Ok(Some(refreshed)) => {
                    let updated = upsert_credential(
                        ...
                        CredentialUpsertInput {
                            ...
                            user_email: selected.user_email.clone(),
    ...
    async fn resolve_credential_input(
        ...
        let existing = forced_id
            .and_then(|id| doc.credentials.iter().find(|item| item.id == id))
            .cloned()
            .or_else(|| {
                input
                    .id
                    .as_deref()
                    .and_then(|id| doc.credentials.iter().find(|item| item.id == id).cloned())
            });
    ...
    async fn build_usage_payload(
        ...
        for credential in credentials {
            let usage = fetch_usage(&credential.access_token)
                .await
